...
REFUSAL_REASONS = (
  ...
  "contradiction",            # contradiction-class edges are NON-TRAVERSABLE for
                              ...
                              # contradicting pair), or when the best path lies
                              # on a cycle containing such an edge. Benign
                              # reciprocal cycles NEVER refuse (they need
                              # visited-set discipline in the walker, not edges)
  "below_floor",              # only used when the caller demands hard answers
                              # (resolve(..., hard=True)); default behavior for
                              # sub-floor results is WEAK_ANSWER, not REFUSE
