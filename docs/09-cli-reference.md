...
- `schema integrity --instance P` — inspect the bound database; exit 1 when strict integrity fails.
...
- `observe --instance P --query Q --resolution-status S --outcome O [...]` — append bounded feedback without modifying graph rules.
- `memory (propose|list|snapshot|review|approve|dispute|supersede|retire) --instance P [...]`
  — manage the append-only typed MemoryLog. Activation requires
  `memory approve --memory-id ID --approve`; `snapshot` returns active orientation only.
