...
DEFAULT_INPUT = INSTANCE_ROOT / "source" / "reasoning-graph.db"
DEFAULT_OUTPUT = INSTANCE_ROOT / "data" / "reasoning-graph.clean.db"
DEFAULT_MANIFEST = INSTANCE_ROOT / "data" / "reasoning-graph.clean.manifest.json"
SOURCE_GLOBS = ("instances/claude_code_tools/source/workflow-declarations.json",)
        ...
        for source in sorted(repo_root.glob(pattern)):
            try:
                ...
                raise MigrationError(
                    f"cannot read tool source {source}: {exc}"
                ) from exc
            for record in _records(document, source):
                ...
                for workflow_id in workflows:
                    if not isinstance(workflow_id, str) or not workflow_id:
                        raise MigrationError(
                            f"invalid workflow ID in {relative_source}"
                        )
...
def validate(conn: sqlite3.Connection) -> dict[str, Any]:
    ...
    if (
        ...
        or foreign_keys
        or wrong_direction
        or duplicate_typed_relationships
    ):
...
def migrate(input_path: Path, output_path: Path, manifest_path: Path) -> dict[str, Any]:
    ...
    if len({input_path, output_path, manifest_path}) != 3:
        raise MigrationError(
            "input, output, and manifest paths must be pairwise distinct"
        )
    ...
    try:
        ...
        temp_manifest_path = Path(manifest_handle.name)
        ...
        if sha256(input_path) != input_hash_before:
            raise MigrationError("immutable input database changed before publication")
