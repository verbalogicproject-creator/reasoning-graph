...
source_node_id/target_node_id/edge_type — the same 'claude' profile as the
bundled clean Instance 0 database) so this is a port, not a redesign: a portable
default DB path and --db/--json were added, while all 7
...
DB_PATH = (
    Path(__file__).parent / "instances" / "claude_code_tools" / "data" / "reasoning-graph.clean.db"
)
