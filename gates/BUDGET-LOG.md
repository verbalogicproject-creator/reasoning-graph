...
| Phase 7 / G8 | PASS | ~within est | 120K | 40 tests (1:1 INVARIANTS), deterministic demo, self-verifying examples, full house doc set (0 OPUS-FILLS markers, README quickstart executes), numpy byte-identical, CHANGELOG hardening pass. RG-4 found+fixed (retire self-sufficiency). Docs bar (NON-NEGOTIABLE) met. |
...
| Phase 8 / G7 (stretch) | PASS | reserve-funded | 80K (reserve) | Second corpus (lowering-ladder domain) built as pure declaration (new graphschema + instance + 5-query fixture); ZERO core edits (CORE-LOCK refreshed to end-of-Phase-7 core first — G3 wrote it prematurely at Phase 3 before loop/+measure/ existed; refresh done transparently with all gates green + committed). ALL 9 GATES G0-G8 PASS. |
