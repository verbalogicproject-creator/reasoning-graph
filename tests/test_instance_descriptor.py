...
DESCRIPTOR = REPO_ROOT / "instances" / "claude_code_tools" / "instance.json"
...
class InstanceDescriptorStaticTests(unittest.TestCase):
    def test_descriptor_has_only_relative_runtime_paths(self) -> None:
        ...
        self.assertEqual(document["adapter"]["argv"][:2], ["python3", "query.py"])
        self.assertEqual(document["adapter"]["argv"][2:], ["--db", "instances/claude_code_tools/data/reasoning-graph.clean.db"])
...
class CanonicalEngineIntegrationTests(unittest.TestCase):
    def test_descriptor_schema_and_database_integrity(self) -> None:
        ...
        instance_root = DESCRIPTOR.parent
        self.assertEqual(instance.db_path, instance_root / "data" / "reasoning-graph.clean.db")
        self.assertEqual(instance.observations_path, instance_root / "data" / "observations.jsonl")
        self.assertTrue((instance_root / "source" / "reasoning-graph.db").is_file())
